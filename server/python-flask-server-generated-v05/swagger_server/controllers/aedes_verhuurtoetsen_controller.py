...
def aedes_verhuurtoets_passendheidstoets_bri_met_kale_huur_post(body):  # noqa: E501
    ...
    if connexion.request.is_json:
        ...
        if len(body.inkomens_ondertekend) > 0 :
            if(body.controleer_handtekening):
                ...
                try :
                    ...
                    # map naar dict
                    inkomens = [Inkomen.from_dict(i) for i in inkomens]
        ...
        # verpakken in jwt token
        toets.jwt = jwt.encode({'data' : {'personen' : prsn, 
                                        'toets_resultaat' : str(toets.toets_resultaat),
                                        'tijdstempel': toets.tijdstempel
                                        }
                                        },
                                        private_key, 
...
def aedes_verhuurtoets_passendheidstoets_bri_segmenten_post(body):  # noqa: E501
    ...
    if connexion.request.is_json:
        ...
        # verpakken in jwt token
        toets.jwt = jwt.encode({'data' : {'personen' : prsn, 
                                        'toets_resultaat_tot_hoge_aftoppingsgrens' : str(toets.toets_resultaat_tot_hoge_aftoppingsgrens),
                                        'toets_resultaat_tot_lage_aftoppingsgrens' : str(toets.toets_resultaat_tot_lage_aftoppingsgrens),
                                        'tijdstempel': toets.tijdstempel
                                        }
                                        },
                                        private_key, 
